...
type MakeLocalDBActions<S, A> = (
  get: () => S,
  set: (action: SetStore<S>) => S,
) => A;
...
class MakeLocalDB<S, A extends ObjectMethods> {
  private store: S;

  private actions: A;

  constructor(initialStore: S, actions: MakeLocalDBActions<S, A>) {
